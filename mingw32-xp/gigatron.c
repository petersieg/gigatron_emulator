				...
				const char *gt1_filename)
{
    ...
    uint8_t gt1Bytes[65536]; // (uint8_t *) malloc(65536 * sizeof(uint8_t));

    /*if (!gt1Bytes) {
        fprintf(stderr, "memory exhausted GT1 malloc\n");
        goto fail_create;
    }*/
    /*    Gt1File :=
     n * ( <hiAddress>
    ...
    fp = fopen(gt1_filename, "r");
    if (!fp) {
        fprintf(stderr, "could not open GT1 file `%s` for reading\n",
                gt1_filename);
        goto fail_create;
    }

    gt1Length = fread(gt1Bytes, sizeof(uint8_t), 65536, fp); // max. 65536 bytes gt1 filesize
    fclose(fp);
	fprintf(stderr, "GT1 filesize: %lu\n", gt1Length);
	int hiStart = 0;
	int loStart = 0;
...
#define posInc      if (++pos >= gt1Length) { break; }
    ...
    while(1) {
        ...
        fprintf(stderr, "Writing %d bytes starting at address 0x%04x\n", segmentSize, startAddress);
        for(int i=0; i<segmentSize; i++) {
            gs->ram[startAddress + i] = gt1Bytes[pos];
            if (i < segmentSize) posInc
        }
        if(gt1Bytes[pos] == 0) {
            posInc
            hiStart = gt1Bytes[pos];
            posInc
            loStart = gt1Bytes[pos];
            pos++;
            if (pos != gt1Length) {
                fprintf(stderr, "Didn't reach end of GT1 file while parsing!\n");
                break;
            } else { 
                // Set virtual program counter
                fprintf(stderr, "Starting program at 0x%02x%02x\n", hiStart, loStart);
                gs->ram[0x16] = loStart - 2;
                gs->ram[0x17] = hiStart;
                gs->ram[0x1a] = loStart - 2;
                gs->ram[0x1b] = hiStart;
                loaded = 1;
            }
            break;
    ...
    if (!loaded) {
        fprintf(stderr, "Error parsing GT1 file!\n");
        goto fail_create;    	
    }
    return (hiStart * 256 + loStart); // pc
